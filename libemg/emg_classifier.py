...
class EMGClassifier:
    ...
    def add_velocity(self, train_windows, train_labels, transfer_function=None):
        ...
        train_labels: np.array
            The labels associated with the train windows. 
        transfer_function: function (optional, default=None)
            A function that re-maps the linear proportionality to some other function (e.g., sigmoid).
        """
        self.velocity = True
    ...
    def _get_velocity(self, window, c):
        if self.th_max_dic and self.th_min_dic:
            ...
            if self.transfer_function is not None:
                ...
                velocity_output = self.transfer_function(velocity_output)
